...
use super::node::{NodeBaseParams, Children};
...
impl MusicNodeParams {
    pub fn read(r: &mut BinaryReader, has_feedback: bool) -> Result<Self> {
        ...
        let stingers = r.read_list_u32(|r| {
            Ok(Stinger {
                trigger_id: r.read_u32()?, segment_id: r.read_u32()?,
                sync_play_at: r.read_u32()?, cue_filter_hash: r.read_u32()?,
                dont_repeat_time: r.read_i32()?, num_segment_look_ahead: r.read_u32()?,
            })
        })?;
        Ok(Self { flags, node_base_params, children, meter_info, meter_info_flag, stingers })
    }
    pub fn write(&self, w: &mut BinaryWriter) {
        ...
        w.write_list_u32(&self.stingers, |w, s| {
            w.write_u32(s.trigger_id); w.write_u32(s.segment_id);
            w.write_u32(s.sync_play_at); w.write_u32(s.cue_filter_hash);
            w.write_i32(s.dont_repeat_time); w.write_u32(s.num_segment_look_ahead);
        });
    }
}
...
impl MusicFade {
    pub fn read(r: &mut BinaryReader) -> Result<Self> {
        Ok(Self { transition_time: r.read_i32()?, fade_curve: r.read_u32()?, fade_offset: r.read_i32()? })
    }
    pub fn write(&self, w: &mut BinaryWriter) {
        w.write_i32(self.transition_time); w.write_u32(self.fade_curve); w.write_i32(self.fade_offset);
    }
}
...
impl MusicTransitionObject {
    pub fn read(r: &mut BinaryReader) -> Result<Self> {
        Ok(Self {
            ...
            fade_in: MusicFade::read(r)?, fade_out: MusicFade::read(r)?,
            play_pre_entry: r.read_u8()?, play_post_exit: r.read_u8()?,
        })
    }
    pub fn write(&self, w: &mut BinaryWriter) {
        w.write_u32(self.segment_id); self.fade_in.write(w); self.fade_out.write(w);
        w.write_u8(self.play_pre_entry); w.write_u8(self.play_post_exit);
    }
}
...
impl MusicTransitionRule {
    pub fn read(r: &mut BinaryReader) -> Result<Self> {
        ...
        let src_rule = MusicTransSrcRule {
            transition_time: r.read_i32()?, fade_curve: r.read_u32()?,
            fade_offset: r.read_i32()?, sync_type: r.read_u32()?,
            cue_filter_hash: r.read_u32()?, play_post_exit: r.read_u8()?,
        };
        let dst_rule = MusicTransDstRule {
            transition_time: r.read_i32()?, fade_curve: r.read_u32()?,
            fade_offset: r.read_i32()?, cue_filter_hash: r.read_u32()?,
            jump_to_id: r.read_u32()?, entry_type: r.read_u16()?,
            play_pre_entry: r.read_u8()?, dest_match_source_cue_name: r.read_u8()?,
        };
        let flag = r.read_u8()?;
        let trans_object = if flag != 0 { Some(MusicTransitionObject::read(r)?) } else { None };
        Ok(Self { src_ids, dst_ids, src_rule, dst_rule, alloc_trans_object_flag: flag, trans_object })
    }
    pub fn write(&self, w: &mut BinaryWriter) {
        w.write_list_u32(&self.src_ids, |w, &id| w.write_u32(id));
        w.write_list_u32(&self.dst_ids, |w, &id| w.write_u32(id));
        w.write_i32(self.src_rule.transition_time); w.write_u32(self.src_rule.fade_curve);
        w.write_i32(self.src_rule.fade_offset); w.write_u32(self.src_rule.sync_type);
        w.write_u32(self.src_rule.cue_filter_hash); w.write_u8(self.src_rule.play_post_exit);
        w.write_i32(self.dst_rule.transition_time); w.write_u32(self.dst_rule.fade_curve);
        w.write_i32(self.dst_rule.fade_offset); w.write_u32(self.dst_rule.cue_filter_hash);
        w.write_u32(self.dst_rule.jump_to_id); w.write_u16(self.dst_rule.entry_type);
        w.write_u8(self.dst_rule.play_pre_entry); w.write_u8(self.dst_rule.dest_match_source_cue_name);
        w.write_u8(self.alloc_trans_object_flag);
        if let Some(obj) = &self.trans_object { obj.write(w); }
    }
}
...
impl MusicTransNodeParams {
    pub fn read(r: &mut BinaryReader, has_feedback: bool) -> Result<Self> {
        ...
        Ok(Self { music_node_params, rules })
    }
    pub fn write(&self, w: &mut BinaryWriter) {
...
impl TrackSrcInfo {
    pub fn read(r: &mut BinaryReader) -> Result<Self> {
        Ok(Self {
            track_id: r.read_u32()?, source_id: r.read_u32()?,
            play_at: r.read_f64()?, begin_trim_offset: r.read_f64()?,
            end_trim_offset: r.read_f64()?, src_duration: r.read_f64()?,
        })
    }
    pub fn write(&self, w: &mut BinaryWriter) {
        w.write_u32(self.track_id); w.write_u32(self.source_id);
        w.write_f64(self.play_at); w.write_f64(self.begin_trim_offset);
        w.write_f64(self.end_trim_offset); w.write_f64(self.src_duration);
    }
}
...
impl ClipAutomation {
    pub fn read(r: &mut BinaryReader) -> Result<Self> {
        ...
        Ok(Self { clip_index, auto_type, graph_points })
    }
    pub fn write(&self, w: &mut BinaryWriter) {
...
impl TrackSwitchParams {
    pub fn read(r: &mut BinaryReader) -> Result<Self> {
        ...
        Ok(Self { group_type, group_id, default_switch, switch_associations })
    }
    pub fn write(&self, w: &mut BinaryWriter) {
...
impl TrackTransParams {
    pub fn read(r: &mut BinaryReader) -> Result<Self> {
        ...
        Ok(Self { src_fade_params, sync_type, cue_filter_hash, dest_fade_params })
    }
    pub fn write(&self, w: &mut BinaryWriter) {
