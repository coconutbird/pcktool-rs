...
impl DialogueEventValues {
    pub fn read(r: &mut BinaryReader) -> Result<Self> {
        ...
        Ok(Self { probability, tree_depth, arguments, tree_data_size, tree_mode, decision_tree })
    }
    ...
    pub fn read(r: &mut BinaryReader) -> Result<Self> {
        ...
        if size > 0 {
            match fx_id {
                ...
                PluginId::WWISE_SILENCE => fx_src_silence_params = Some(FxSrcSilenceParams::read(r)?),
        ...
        for _ in 0..num_bank_data {
            media_map.push(MediaMapEntry { index: r.read_u8()?, source_id: r.read_u32()? });
        }
        let initial_rtpc = InitialRtpc::read(r)?;
        let num_init = r.read_u16()? as usize;
        let mut rtpc_init_list = Vec::with_capacity(num_init);
        for _ in 0..num_init {
            rtpc_init_list.push(RtpcInit { param_id: r.read_u8()?, init_value: r.read_f32()? });
        }
        Ok(Self { fx_id, size, plugin_param, fx_src_silence_params, delay_fx_params, raw_plugin_data, media_map, initial_rtpc, rtpc_init_list })
    }
    pub fn write(&self, w: &mut BinaryWriter) {
        ...
        if self.size > 0 {
            match self.fx_id {
                PluginId::WWISE_COMPRESSOR => { if let Some(p) = &self.plugin_param { p.write(w); } }
                PluginId::WWISE_SILENCE => { if let Some(p) = &self.fx_src_silence_params { p.write(w); } }
                PluginId::WWISE_DELAY => { if let Some(p) = &self.delay_fx_params { p.write(w); } }
                _ => { if let Some(d) = &self.raw_plugin_data { w.write_bytes(d); } }
            }
        }
        w.write_u8(self.media_map.len() as u8);
        for e in &self.media_map { w.write_u8(e.index); w.write_u32(e.source_id); }
        self.initial_rtpc.write(w);
        w.write_u16(self.rtpc_init_list.len() as u16);
        for i in &self.rtpc_init_list { w.write_u8(i.param_id); w.write_f32(i.init_value); }
    }
}
